...
public class AgregarEvento extends AppCompatActivity {
    ...
    public void clickAceptarEvento(View v)
    {
        if (validarDatos())
        {
            ...
            if (b)
            {
                Toast.makeText(getApplicationContext(),"Evento Registrado Satisfactoriamente",Toast.LENGTH_LONG);
                this.finish();
            }
            else
            {
                Toast.makeText(getApplicationContext(),"Evento no Ingresado INTALO MAS TARDE",Toast.LENGTH_LONG);
            }
        }
        else
        {
